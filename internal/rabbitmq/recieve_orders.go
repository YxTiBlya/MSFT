...
func RecieveOrder(c *cfg.Config) {
	...
	if err != nil {
		log.Fatal("rabbitmq: error to declare queue:\n", err.Error())
	}
	...
	if err != nil {
		log.Fatal("rabbitmq: error to bind queue:\n", err.Error())
	}

	msgs, err := Chann.Consume(q.Name, "", true, false, false, false, nil)
	if err != nil {
		log.Fatal("rabbitmq: error to consume init:\n", err.Error())
	}

	var forever chan struct{}
	log.Println("starting listening order queue at rabbitmq")
	go func() {
		for d := range msgs {
